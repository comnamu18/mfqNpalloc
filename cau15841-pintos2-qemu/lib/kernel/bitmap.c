  ...
  switch( pallocator ) {
    ...
    case 2: //BestFit
      if (cnt <= b->bit_cnt) {
        ...
        int count, min; // min and count mean how many empty bits after start + cnt
        bool escapeFlag, returnFlag;
        // initialize
        escapeFlag = true;
        returnFlag = false;
        min = 0;
        ...
        for (i = start; i <= last; i++) {
          if (!bitmap_contains (b, i, cnt, !value)) {
            ...
            // initalize
            count = 0;
            // count how many 0bit after start + cnt
            for ( j = cnt; j <= last; j++ ) {
              if (bitmap_test(b, i + j)) {
                break;
              }
              count++;
            }
            // If this is first solution or new solution is better than before
            if ( escapeFlag || count < min ) {
              min = count;
              n = i;
              // Since we already check fragmentation number, we can skip it
              i = i + count;
              escapeFlag = false;
            }
          }
